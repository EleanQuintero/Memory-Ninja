...
import { useLoad } from "@/app/dashboard/flashcards/hooks/useLoad";
...
export default function FlashCardsPage() {
  ...
  const { filteredFlashcards } = useFilterFlashcards({ themeToFilter: selectedTheme });
  
  // Obtener datos del usuario
  const user_id = user?.id; 
  const userName = (user?.firstName?.slice(0,5)) + " " + (user?.lastName?.slice(0,8))
  
  // iniciamos carga de datos
  const { loading, error } = useUIState()

  // Iniciar sincronización
  useLoad(user_id as string)
  useFlashcardSync(user_id as string);

  // Manejar estados de carga y error
  if (loading) {
    return (
      <LoadingModal message="Cargando tus flashcards..." isLoading={loading} />
    );
  }
  ...
  if (error)  {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        <h2 className="text-red-500 text-2xl mb-4">Error al cargar las flashcards</h2>
  ...
  return (
    <section className="container mx-auto p-4">
      ...
      <div className="mb-6">
        ...
        <ThemeSelector onThemeChange={setSelectedTheme} selectedTheme={selectedTheme} />
      </div>
